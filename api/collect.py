...
Run this on a schedule (Render cron / launchd), never from a web request.
"""
...
from api.catalog import CATALOG, IndicatorDef, determine_level  # noqa: E402
...
def _load_class(filename: str, class_name: str):
    """Load a collector class straight from its file, skipping collectors/__init__.py
    (which imports every collector and fails on optional dependencies)."""
    base_path = SRC / "collectors" / "base.py"
    ...
    module_name = f"collectors.{filename[:-3]}"
    mod = sys.modules.get(module_name)
    if mod is None:
        spec = importlib.util.spec_from_file_location(module_name, str(SRC / "collectors" / filename))
        mod = importlib.util.module_from_spec(spec)
...
def _config():
    ...
    return ConfigLoader(str(ROOT / "config" / "config.yaml"))
